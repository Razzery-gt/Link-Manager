...
# Стандартные ссылки
default_links = {
    "открыть_браузер": {"url": "https://example.com", "date_added": str(datetime.now())},
    "гугл": {"url": "https://www.google.com", "date_added": str(datetime.now())},
    "яндекс": {"url": "https://www.yandex.ru", "date_added": str(datetime.now())}
}
    ...
    if os.path.exists(LINKS_FILENAME):
        try:
            with open(LINKS_FILENAME, 'r', encoding='utf-8') as f:
                links = json.load(f)
                # Преобразование старых форматов в новый формат с датой добавления
...
def show_available_keys(links):
    if links:
        ...
        for index, key in enumerate(links.keys(), 1):
            print(f"{index}. {Fore.YELLOW}{key} - {links[key]['url']} (Добавлено: {links[key]['date_added']})")
    else:
...
def search_links(links, query):
    ...
    if found_links:
        ...
        for key, data in found_links.items():
            print(f"{Fore.YELLOW}{key}: {data['url']} (Добавлено: {data['date_added']})")
    else:
